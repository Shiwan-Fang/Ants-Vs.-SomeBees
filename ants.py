...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            is_placeant_container = isinstance(place.ant, ContainerAnt)
            if not is_self_container and is_placeant_container and not place.ant.contained_ant:
                place.ant.contain_ant(self)
            elif is_self_container and not is_placeant_container and not self.contained_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place is not beehive:
            if current_place.bees and self.min_range <= thrower_range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # make the fireant take damage(insect class's reduce_armor) from the bees
        fire_place = self.place.bees[:]
        Ant.reduce_armor(self, amount)
        # evert bee that is in the same place as the ant takes amount damage(insect class's reduce_armor)
        # if fireant's armor is >0, otherwise they take amount+damage damage(insect class's reduce_armor)
        if self.armor:
            for bee in fire_place:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if not self.digesting:
            if self.place.bees:
                hungry_place_bee = rANTdom_else_none(self.place.bees)
                return self.eat_bee(hungry_place_bee)
        else:
            self.digesting -= 1
        # END Problem 6
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        self.real_queen = True if QueenAnt.counter_queen == 0 else False
        QueenAnt.counter_queen += 1
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.real_queen:
            ScubaThrower.reduce_armor(self, amount=self.armor)
            QueenAnt.counter_queen -= 1
        else:
            ScubaThrower.action(self, gamestate)
            next_place = self.place.exit
            while next_place:
                if next_place.ant and not next_place.ant.damage_buffed:
                    next_place.ant.damage = next_place.ant.damage * 2
                    next_place.ant.damage_buffed = True
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        "*** YOUR CODE HERE ***"
        ninja_place = self.place.bees[:]
        for bee in ninja_place:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if not self.contained_ant and isinstance(other, ContainerAnt):
            return True
        return False
        # END Problem Optional 2
